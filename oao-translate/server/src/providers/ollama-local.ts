...
export class OllamaLocalProvider implements TranslationProvider {
  ...
  processTranscriptText(text: string): void {
    const transcript = text.trim();
    if (!transcript) return;
    void this.publishTranscriptAndTranslations(transcript);
  }
  ...
  private async publishTranscriptAndTranslations(transcript: string): Promise<void> {
  ...
  private getTargetLanguages(): string[] {
    ...
    return [this.languages.target || "en"];
  }

  private async translate(text: string, targetCode: string): Promise<string> {
    const target = this.resolveTargetLabel(targetCode);
    const prompt = `You are a professional simultaneous interpreter. Translate the following into ${target}. Return only the translation without notes.\n\n${text}`;
